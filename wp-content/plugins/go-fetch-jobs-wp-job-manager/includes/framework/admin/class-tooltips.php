    ...
    class BC_ToolTips
    {
        ...
        public function _enqueue_css_js()
        {
            ...
            if ( self::supports_wp_pointer() ) {
                ...
                $custom_styles = '' . '.bc-wp-pointer .wp-pointer-content{ background-color: #444; color: #fff; padding: 0; }' . '.bc-wp-pointer .wp-pointer-content a{ color: #0492CA } ' . '.bc-wp-pointer .wp-pointer-content code { background-color: rgb(88, 88, 88); font-size: 11px; }' . '.bc-tip, .bc-wp-pointer .wp-pointer-arrow-inner{ cursor: help; }' . '.wp-pointer-left .wp-pointer-arrow-inner { border-right-color: #444; }' . '.wp-pointer-right .wp-pointer-arrow-inner { border-left-color: #444; }';
...
<?php 
		...
		<script type="text/javascript">
			jQuery( function($) {
				...
				if ( <?php 
            echo  ( self::supports_wp_pointer() ? '1' : '0' ) ;
            ?>
 ) {

					$(document).on( 'mouseenter mouseleave', "<?php 
            echo  $this->selector ;
            ?>
", function(ev) {
            ...
            echo  $this->selector ;
            ?>
").on( 'mouseenter mouseleave', function( ev ) {
						$(this).addClass( hover_class );
