...
class scbForms
{
    ...
    {
        return html( 'form method=\'post\' action=\'\'', $content, wp_nonce_field(
            $nonce,
            '_wpnonce',
            $referer = true,
            $echo = false
    ...
    {
        return html( 'tr', html( 'th scope=\'row\'', $title ), html( 'td', $content ) );
    }
...
abstract class scbFormField implements  scbFormField_I 
{
     ...
     * @param mixed  $value
     */
    protected abstract function _set_value( &$args, $value );
     ...
     * @param array $args
     */
    protected abstract function _render( $args );
    ...
    private static function _expand_choices( &$args )
    {
        ...
        if ( !empty($choices) && !self::is_associative( $choices ) ) {
            ...
            if ( is_array( $args['desc'] ) ) {
                $choices = array_combine( $choices, $args['desc'] );
                // back-compat
                $args['desc'] = false;
            } else {
                if ( !isset( $args['numeric'] ) || !$args['numeric'] ) {
...
abstract class scbSingleChoiceField extends scbFormField
{
     ...
     * @return string
     */
    protected abstract function _render_specific( $args );
