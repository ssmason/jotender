...
class JR_Registration extends APP_View {
	...
	function maybe_enqueue_js_styles( $action ) {
	...
	function recaptcha_verify( $errors ) {
...
class JR_Jobs_Archive extends APP_View {
	...
	function jobs_frontpage( $wp_query ) {
...
class JR_Dashboard extends APP_View {
	...
	function template_include( $path ) {
		if ( ! is_user_logged_in() || ( ! current_user_can('can_submit_resume') && ! current_user_can('can_submit_job') ) ) {
			appthemes_add_notice('no-access', __( 'You don\'t have permissions to access the Dashboard.<br/>You must be a Job Lister or Job Seeker. Please contact the site owner for help.' ) );
...
class JR_Resume_Edit extends APP_View {
	...
	function condition() {
		global $wp_query;
		return isset( $wp_query->queried_object ) && is_page( JR_Resume_Edit_Page::get_id() );
	}
	...
	function template_redirect() {
		...
		if ( false !== get_query_var( 'edit', false ) && ! get_query_var('edit') ) {
			wp_die( __('Invalid resume ID.', APP_TD ) );
		}

		if ( ! current_user_can('can_submit_resume') ) {
			wp_die( __( "You don't have permissions to submit resumes.", APP_TD ) );
		}

		if ( get_query_var('edit') && ! current_user_can( 'edit_resume', (int) get_query_var('edit') )  ) {
			wp_die( __( 'Cheatin&#8217; uh?', APP_TD ) );
		}
